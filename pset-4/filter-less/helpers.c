...
void sepia(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            new_red = round((0.393 * image[i][j].rgbtRed) + (0.769 * image[i][j].rgbtGreen) +
                            (0.189 * image[i][j].rgbtBlue));
            new_green = round((0.349 * image[i][j].rgbtRed) + (0.686 * image[i][j].rgbtGreen) +
                              (0.168 * image[i][j].rgbtBlue));
            new_blue = round((0.272 * image[i][j].rgbtRed) + (0.534 * image[i][j].rgbtGreen) +
